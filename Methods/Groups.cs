...
namespace OpenVkNetApi.Methods
{
    public class Groups
    {
        ...
        /// </summary>
        /// <param name="GroupIds">id групп, которые будут получены</param>
        /// <param name="Offset">Смещение, необходимое для выборки определённого подмножества результатов поиска. По умолчанию — 0.</param>
        /// <param name="Count">кол-во групп</param>
        /// <param name="Fields">Список дополнительных полей, которые необходимо вернуть.</param>
        public async Task<Group[]> GetById(string GroupIds = "", int Offset = 0, int Count = 500, string Fields = "verified,has_photo,photo_max_orig,photo_max,photo_50,photo_100,photo_200,photo_200_orig,photo_400_orig,members_count,site,description,contacts,can_post,is_member")
        {
            return JsonConvert.DeserializeObject<dynamic>(await api.GetRequest($"{InstanceUrl}/method/Groups.getById?access_token={AccessToken}&group_ids={GroupIds}&offset={Offset}&count={Count}&fields={Fields}")).response.ToObject(typeof(Group[]));
        }
        /// <summary>
        ...
        {
            await api.GetRequest($"{InstanceUrl}/method/Groups.join?access_token={AccessToken}&group_id={GroupId}");
        }
        /// <summary>
        ...
        {
            await api.GetRequest($"{InstanceUrl}/method/Groups.leave?access_token={AccessToken}&group_id={GroupId}");
        }
    }
}
