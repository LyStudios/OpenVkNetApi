...
Удаляет отметку «Мне нравится» с указанного объекта (поста, комментария, фотографии и т.д.).
...
```csharp
var result = await api.Likes.DeleteAsync(type: "post", ownerId: 1, itemId: 12);
